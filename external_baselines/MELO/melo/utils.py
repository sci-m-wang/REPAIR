...
def tokenize_qa(batch, tokenizer, device, **kwargs):
    ...
    # Concatenate input and output for causal LM
    full_sequences = [f"{inp} {out}" for inp, out in zip(input_sequences, output_sequences)]
    
    # Tokenize inputs only to get prompt lengths
    ...
    num_input_toks = [int((i != tokenizer.pad_token_id).sum()) for i in input_encoding.input_ids]
    ...
    for i in range(len(input_sequences)):
        labels[i][:num_input_toks[i]] = mask_token
    labels[labels == tokenizer.pad_token_id] = mask_token
