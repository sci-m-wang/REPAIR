...
def F1_ACC(alg, batch):
    try:
        preds = alg.generate(batch["input_ids"], max_new_tokens=20).squeeze()
...
def ACC(preds, batch, tok):
    ...
    min_len = min(len(decode_labels), len(decode_preds))
    decode_labels = decode_labels[:min_len]
    decode_preds = decode_preds[:min_len]
    
    count = 0.
    for pred, label in zip(decode_preds, decode_labels):
        if pred.strip() == label.strip():
            count = count + 1
    return count / len(decode_preds) if len(decode_preds) > 0 else 0.0
