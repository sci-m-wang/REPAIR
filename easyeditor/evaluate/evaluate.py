...
def compute_edit_quality(
    ...
    # 设置 eval 标记：rewrite cursor
    if hasattr(model, 'get_adapter_layer'):
        try:
            layer = model.get_adapter_layer()
            setattr(layer, 'eval_tag', 'rewrite')
            setattr(layer, 'eval_prompt', rewrite_prompts if isinstance(rewrite_prompts, str) else str(rewrite_prompts))
        except Exception:
            pass
    ...
    if hasattr(model, 'get_adapter_layer'):  #cursor
        try:
            layer = model.get_adapter_layer()
            setattr(layer, 'eval_tag', None)
            setattr(layer, 'eval_prompt', None)
        except Exception:
            pass
    ...
    if rephrase_prompts is not None:
        # 设置 eval 标记：rephrase cursor
        if hasattr(model, 'get_adapter_layer'):
            try:
                layer = model.get_adapter_layer()
                setattr(layer, 'eval_tag', 'rephrase')
                setattr(layer, 'eval_prompt',
                        rephrase_prompts if isinstance(rephrase_prompts, str) else str(rephrase_prompts))
            except Exception:
                pass
        ret.update(
    ...
    if 'locality' in record.keys() and any(record['locality']):
        for locality_key in record['locality'].keys():
            # 设置 eval 标记：locality cursor
            if hasattr(model, 'get_adapter_layer'):
                try:
                    layer = model.get_adapter_layer()
                    setattr(layer, 'eval_tag', f'locality:{locality_key}')
                    setattr(layer, 'eval_prompt', record['locality'][locality_key]['prompt'])
                except Exception:
                    pass
            ret['locality'].update(
...
def _generate_text_for_prompt(model, tok, prompt, device, hparams: typing.Optional[HyperParams] = None, target_new: typing.Optional[str] = None) -> str:
    try:
        if hparams is not None and hasattr(hparams, 'use_chat_template') and hparams.use_chat_template:
            messages = [
        ...
        device_str = device if (isinstance(device, str) and device.startswith('cuda')) else f"cuda:{device}"
        input_ids = tok.encode(text, return_tensors="pt").to(device_str)
        with torch.no_grad():
            max_tokens = 128
            if isinstance(target_new, str):
                try:
                    max_tokens = max(32, min(256, 2 * len(tok.encode(target_new))))
                except Exception:
                    pass
            pad_id = None
            try:
                ...
                pass
            if pad_id is None and hasattr(model, 'config') and hasattr(model.config, 'eos_token_id'):
        ...
        return tok.decode(generated_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
    except Exception:
        return ""
